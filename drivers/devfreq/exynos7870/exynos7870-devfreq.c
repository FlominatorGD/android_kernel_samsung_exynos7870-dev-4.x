 ...
 *
 * Samsung Exynos7870 clock/regulator devfreq driver
 *
...
#include <soc/samsung/exynos7870-devfreq.h>
...
int register_exynos7870_devfreq_init_prepare(enum exynos_devfreq_type type,
...
s32 exynos7870_devfreq_get_opp_idx(struct exynos_devfreq_opp_table *table,
...
u32 get_target_exynos7870_devfreq_rate(enum exynos_devfreq_type type, char *name, u32 freq)
{
	u32 target_rate = 0;
...
int exynos7870_devfreq_sync_voltage(enum exynos_devfreq_type type, bool turn_on)
{
	struct exynos_devfreq_data *data;
				...
				unsigned long *target_freq, u32 flags)
{
	...
	*target_freq = dev_pm_opp_get_freq(target_opp);
	...
	target_idx = exynos7870_devfreq_get_opp_idx(data->opp_list, data->max_state,
							*target_freq);
	if (target_idx < 0) {
		ret = -EINVAL;
		goto out;
	...
	if (data->use_switch_clk) {
		...
		if (data->ops.get_switch_voltage) {
			...
			switch_volt = data->switch_volt;
				...
				switch_opp = devfreq_recommended_opp(dev, &switch_freq,
									 flags);
					...
					struct devfreq_dev_status *stat)
{
	...
	if (data->gov_type == SIMPLE_EXYNOS) {
		...
		cur_idx = exynos7870_devfreq_get_opp_idx(data->opp_list, data->max_state,
...
static int exynos_devfreq_probe(struct platform_device *pdev)
{
	...
	data->old_idx = exynos7870_devfreq_get_opp_idx(data->opp_list, data->max_state,
							data->old_freq);
	if (data->old_idx < 0) {
		ret = -EINVAL;
		goto err_old_idx;
...
static struct platform_device_id exynos_devfreq_driver_ids[] = {
	{
		.name		= EXYNOS7870_DEVFREQ_MODULE_NAME,
	},
	{},
};
...
static const struct of_device_id exynos_devfreq_match[] = {
	{
		.compatible	= "samsung,exynos7870-devfreq",
	},
	{},
};
...
MODULE_DESCRIPTION("Samsung Exynos7870 clock/regulator devfreq driver");
MODULE_LICENSE("GPL");
