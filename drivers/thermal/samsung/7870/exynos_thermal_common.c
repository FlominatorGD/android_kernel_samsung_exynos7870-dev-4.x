			...
			struct thermal_cooling_device *cdev)
{
	...
	for (i = 0; i < tab_size; i++) {
		...
		if(data->d_type == CLUSTER0 || data->d_type == CLUSTER1) {
			unsigned int cluster_cpu = 0;

			for_each_possible_cpu(cluster_cpu) {
				if (cpu_topology[cluster_cpu].cluster_id == data->d_type)
					break;
			}

			ret = cpufreq_get_policy(&policy, cluster_cpu);
			if (ret) {
				dev_warn(data->dev,
					 "cpufreq policy for %s not available, skip freq clamp\n",
					 data->name);
			} else {
			...
			unsigned long *temp)
{
	...
	*temp = *temp * MCELSIUS;
	...
	if (th_zone->sensor_conf->d_type == CLUSTER0 || th_zone->sensor_conf->d_type == CLUSTER1) {
		...
		*temp = max(cpu_max_temp[CLUSTER0], cpu_max_temp[CLUSTER1]);
	}

	/*
	 * The 4.14 cpu/gpu/isp cooling devices take the temperature through
	 * their set_cur_temp cooling operation instead of the
	 * cpufreq_set_cur_temp()/gpufreq_set_cur_temp() global hooks.
	 */
	for (i = 0; i < th_zone->cool_dev_size; i++) {
		if (th_zone->cool_dev[i] &&
		    th_zone->cool_dev[i]->ops->set_cur_temp)
			th_zone->cool_dev[i]->ops->set_cur_temp(
				th_zone->cool_dev[i], suspended, *temp / 1000);
	}
...
static int exynos_throttle_cpu_hotplug(struct thermal_zone_device *thermal)
{
	...
	cpufreq_device = th_zone->cool_dev_size > 0 && th_zone->cool_dev[0] ?
		(struct cpufreq_cooling_device *)th_zone->cool_dev[0]->devdata :
		NULL;
	...
	if (sensor_conf->cooling_data.freq_clip_count > 0) {
		if (sensor_conf->d_type == CLUSTER0 || sensor_conf->d_type == CLUSTER1) {
			for_each_possible_cpu(cpu) {
				...
				/*
				 * The 4.14 cpufreq cooling API takes the
				 * cpufreq policy instead of a cpumask, so
				 * hold a reference to the policy of the
			...
			if (!th_zone->cool_dev[th_zone->cool_dev_size])
				dev_warn(sensor_conf->dev,
					 "cpufreq cooling device for %s not available\n",
					 sensor_conf->name);
		...
		if (IS_ERR(th_zone->cool_dev[th_zone->cool_dev_size])) {
			...
			ret = -EINVAL;
			goto err_unregister;
		} else if (th_zone->cool_dev[th_zone->cool_dev_size]) {
...
void exynos_unregister_thermal(struct thermal_sensor_conf *sensor_conf)
{
	...
	for (i = 0; i < th_zone->cool_dev_size; i++) {
		if (sensor_conf->d_type == CLUSTER0 || sensor_conf->d_type == CLUSTER1) {
			struct cpufreq_cooling_device *pcdev;

			pcdev = (struct cpufreq_cooling_device *)th_zone->cool_dev[i]->devdata;
			if (pcdev)
				cpufreq_cpu_put(pcdev->policy);
